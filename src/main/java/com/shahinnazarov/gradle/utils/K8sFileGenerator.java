...
public class K8sFileGenerator {
    ...
    public String generateImagePullPolicy(K8sImagePullPolicies imagePullPolicy) {
        return "imagePullPolicy: ".concat(imagePullPolicy.getName()).concat("\n");
    }
    ...
    public String generateSelector(K8sSelector selector, int size) {
        ...
        if (selector.getMatchLabels() != null) {
            result.append(addTab(generateMatchLabels(), size + 1));
            selector.getMatchLabels().forEach((k, v) -> {
                result.append(addTab(generateKeyValue(k, v), size + 2));
            });
        }

        if (selector.getMatchExpressions() != null) {
            result.append(addTab(generateMatchExpressions(), size + 1));
        ...
        if (podTemplateSpec.getContainers() != null) {
            ...
            podTemplateSpec.getContainers().forEach(container -> {
                ...
                if (container.getImagePullPolicy() != null) {
                    result.append(addTab(generateImagePullPolicy(container.getImagePullPolicy()), 4));
                    selected.set(true);
                }
    ...
    public String generateDeploymentSpec(K8sDeploymentSpec k8sDeploymentSpec) {
        ...
        if (k8sDeploymentSpec.getSelector() != null) {
            result.append(generateSelector(k8sDeploymentSpec.getSelector()));
        }
